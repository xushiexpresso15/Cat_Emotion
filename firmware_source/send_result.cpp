...
el_err_code_t send_bytes(const char* buffer, size_t size) {
    DEV_UART* uart1 = hx_drv_uart_get_dev(USE_DW_UART_1);
    if (uart1) {
        uart1->uart_write(buffer, size);
    }
    DEV_UART* uart0 = hx_drv_uart_get_dev(USE_DW_UART_0);
    if (uart0) {
        uart0->uart_write(buffer, size);
    }
    return EL_OK;
}
...
el_err_code_t read_bytes(char* buffer, size_t size) {
    ...
    while (size) {
        ...
        size_t r = 0;
        if (uart0) {
            r = uart0->uart_read(buffer + pos_of_bytes, bytes_to_read);
        }
        if (r == 0 && uart1) {
            r = uart1->uart_read(buffer + pos_of_bytes, bytes_to_read);
        }
        read += r;
...
el_err_code_t read_bytes_nonblock(char* buffer, size_t size) {
    ...
    while (size) {
        ...
        size_t r = 0;
        if (uart0) {
            r = uart0->uart_read_nonblock(buffer + pos_of_bytes, bytes_to_read);
        }
        if (r == 0 && uart1) {
            r = uart1->uart_read_nonblock(buffer + pos_of_bytes, bytes_to_read);
        }
        read += r;
        pos_of_bytes += bytes_to_read;
        size -= bytes_to_read;
...
{
    // Disabled to prevent rebooting during streaming
}
...
std::string img_2_json_str(el_img_t* img) {
    ...
    static char s_img_b64_buf[16384];
    if (img->size > 12000) [[unlikely]] {
        return std::string("\"image\": \"\"");
    }

    std::memset(s_img_b64_buf, 0, sizeof(s_img_b64_buf));
    el_base64_encode(img->data, img->size, s_img_b64_buf);

    return concat_strings("\"image\": \"", s_img_b64_buf, "\"");
}
...
void event_reply_fast(const std::string& meta_json, const std::string& img_json) {
    static char s_uart1_dma_buf[2][16384] __attribute__((aligned(32)));
    static uint8_t s_dma_idx = 0;
    ...
    if (uart1) {
        if (s_dma_in_use) {
            ...
            while (uart1->uart_control(UART_CMD_GET_TX_DMA_DONE, &dma_done) == 0 && dma_done == -1) {
        ...
        const auto& ss_full{concat_strings("\r{\"type\": 1, \"name\": \"INVOKE\", \"code\": 0, \"data\": {\"count\": 0",
                                           meta_json,
                                           ", ",
                                           img_json,
                                           "}}\n")};
        size_t full_sz = ss_full.size();
        if (full_sz < sizeof(s_uart1_dma_buf[0])) {
            char* target_buf = s_uart1_dma_buf[s_dma_idx];
            s_dma_idx ^= 1;
            memcpy(target_buf, ss_full.c_str(), full_sz);
            hx_CleanDCache_by_Addr((volatile void *)target_buf, full_sz);
            int32_t ret = uart1->uart_write_udma ? uart1->uart_write_udma(target_buf, full_sz, NULL) : -1;
            if (ret == 0) {
                s_dma_in_use = true;
            } else {
                uart1->uart_write(target_buf, full_sz);
                s_dma_in_use = false;
            }
        } else {
            uart1->uart_write(ss_full.c_str(), full_sz);
            s_dma_in_use = false;
        }
    }
    ...
    DEV_UART* uart0 = hx_drv_uart_get_dev(USE_DW_UART_0);
    if (uart0) {
        const auto& ss_meta{concat_strings("\r{\"type\": 1, \"name\": \"INVOKE\", \"code\": 0, \"data\": {\"count\": 0",
                                           meta_json,
                                           "}}\n")};
        uart0->uart_write(ss_meta.c_str(), ss_meta.size());
    }
}
